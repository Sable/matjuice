...
function drv_prime(scale) {
    ...
    // %tic
    mc_t0 = prime_total(scale);
    mc_disp(mc_t0);
    // %toc
}

function prime_total(n) {
    var j;
    var mc_t2;
    var i;
    var mc_t4;
    ...
    for (i = mc_t8; i <= n; i = i + 1) {
        ...
        for (j = mc_t7; j <= mc_t4; j = j + 1) {
            mc_t2 = mc_mod_SS(i, j);
            mc_t6 = 0;
            mc_t5 = mc_eq_SS(mc_t2, mc_t6);
            if (mc_t5) {
                prime = 0;
                // %break;
            }
        }
        total = mc_plus_SS(total, prime);
    }
    
    return total;
}
var t1 = performance.now();drv_prime(1000000);var t2 = performance.now();print(t2 - t1);